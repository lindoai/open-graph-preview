...
import { visualStyles, visualContainerMarkup, viewToggleMarkup, visualScript } from './visuals';
...
export function renderTextToolPage(options: TextToolOptions) {
  ...
  const hasVisual = Boolean(slug);

  return `<!doctype html><html><head><meta charset="utf-8" /><meta name="viewport" content="width=device-width, initial-scale=1" /><title>${escapeHtml(options.title)}</title><style>${baseStyles()}${hasVisual ? visualStyles() : ''}</style>${turnstileScriptTag()}</head><body><div class="wrap"><div class="card"><h1>${escapeHtml(options.title)}</h1><p>${escapeHtml(options.description)}</p><form id="tool-form"><div class="${rowClass}"><input id="url" type="url" placeholder="https://example.com" required />${formatSelect}</div>${turnstileMarkup()}<button type="submit">${escapeHtml(options.buttonLabel)}</button></form></div><div class="card result"><div class="result-head"><div id="status" class="muted">No request yet.${hasVisual ? viewToggleMarkup() : ''}</div><button id="copy-btn" type="button" class="copy-btn">Copy</button></div>${hasVisual ? visualContainerMarkup() : ''}<pre id="output">${escapeHtml(options.sample)}</pre></div></div><script>${renderTextToolScript(options)}${hasVisual ? visualScript(slug) : ''}</script></body></html>`;
}
...
function renderTextToolScript(options: TextToolOptions) {
  ...
  const hasVisual = Boolean(options.toolSlug);
  const visualCall = hasVisual ? `try{renderVisual(JSON.parse(text));}catch(e){}` : '';

  let pretty: string;
  if (options.formatOptions?.some((option) => option.value === 'markdown')) {
    pretty = `if(format==='json'){output.textContent=JSON.stringify(JSON.parse(text),null,2);${visualCall}}else{output.textContent=text;}`;
  } else {
    pretty = `output.textContent=JSON.stringify(JSON.parse(text),null,2);${visualCall}`;
  }
